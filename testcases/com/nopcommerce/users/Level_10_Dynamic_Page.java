...
public class Level_10_Dynamic_Page extends BaseTest {
    ...
    @Test
    public void User_05_Switch_Page_Dynamic_Locator() {
        addressPage = (UserAddressPO) myProductReviewsPage.openNavigationLinkPages("customer-address");

        ordersPage = (UserOrdersPO) addressPage.openNavigationLinkPages("customer-order");

        downloadableProductsPage = (UserDownloadableProductsPO) ordersPage.openNavigationLinkPages("downloadable-products");

    }

    @Test
    public void User_06_Switch_Page_Dynamic_Locator() {

        downloadableProductsPage.openNavigationLinkPage("customer-address");
        addressPage = PageGenerator.getUserAddressPage(driver);

        addressPage.openNavigationLinkPage("customer-order");
        ordersPage = PageGenerator.getUserOrdersPage(driver);

        ordersPage.openNavigationLinkPage("downloadable-products");
        downloadableProductsPage = PageGenerator.getUserDownloadableProductsPage(driver);
